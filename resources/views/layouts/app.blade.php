...
<body class="bg-gray-100 min-h-screen flex flex-col">
    ...
    <div class="flex flex-1 overflow-hidden">
        <!-- SIDEBAR MENU -->
        <aside x-show="menuOpen" x-transition.duration.300ms
               class="w-64 bg-white shadow-md p-4 overflow-y-auto"
               style="display: none;">
            <nav class="space-y-2">
                <a href="{{ route('dashboard') }}"
                   class="block p-2 rounded hover:bg-blue-100 font-medium">
                   🏠 Dashboard
                </a>
                ...
                <div x-data="{ open: false }" class="border-t border-gray-200 pt-2">
                    ...
                    <div x-show="open" x-transition.duration.300ms class="ml-5 mt-1 space-y-1" style="display: none;">
                        <a href="{{ route('students.index') }}" class="block px-2 py-1 text-sm text-gray-700 hover:bg-blue-100 rounded">
                            📋 Daftar Siswa
                        </a>
                        <a href="{{ route('students.scores') }}" class="block px-2 py-1 text-sm hover:bg-blue-100 rounded">
                            📊 Nilai Siswa
                        </a>
                        <a href="{{ route('students.tryoutquick') }}" class="block px-2 py-1 text-sm hover:bg-blue-100 rounded">
                            🚀 Try Out Quick
                        </a>
                        <a href="{{ route('students.tryoutbasic') }}" class="block px-2 py-1 text-sm hover:bg-blue-100 rounded">
                            ⚡ TKA SIAP Basic
                        </a>
                        <a href="{{ route('students.tryoutfull') }}" class="block px-2 py-1 text-sm hover:bg-blue-100 rounded">
                            ⭐ TKA SIAP Full
                        </a>
